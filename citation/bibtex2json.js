...
var parseBib = function (file, enc, csl, outfile, callback) {
    ...
    try {
        ...
        var filecontent = "{\n";
        while (match !== null) {
            if (!String(match).toLowerCase().match(/(@comment|@string|@preamble)/g)) {
                filecontent += '"' + match[1] + '":"[@' + match[1] + ']",\n';
            }
            match = regex.exec(bib_file);
        }
        filecontent = filecontent.substr(0, filecontent.length - 2) + "\n}";
        fs.writeFile("./tmp/keys.txt", filecontent, function (err) {
        ...
        fs.writeFile(outfile + "/fkeys.json", "{}", function (err) {
            if (err) return console.log(err);
        });
        fs.writeFile(outfile + "/fbib.json", "{}", function (err) {
            if (err) return console.log(err);
        });
        callback();
    }
...
var extractFormatedKeys = function (filepath) {
    ...
    return fs.writeFile(filepath + "/fkeys.json", result, function (err) {
        if (err) return console.log(err);
    });
};
...
var extractFormatedBib = function (filepath) {
    ...
    if (!fs.existsSync(filepath)) fs.mkdirSync(filepath);
    return fs.writeFile(filepath + "/fbib.json", filecontent, function (err) {
        if (err) return console.log(err);
    });
};
