...
export function criteria(files, done) {
    let config = existsSync(files.config) ? yaml2json.safeLoad(readFileSync(files.config, "utf8")) : {};
    let autoConfig = existsSync(files.autoConfig) ? yaml2json.safeLoad(readFileSync(files.autoConfig, "utf8")) : {};
