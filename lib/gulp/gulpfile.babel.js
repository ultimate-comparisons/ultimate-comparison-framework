...
gulp.task('determineColors', function (done) {
    ...
    data.forEach((map) => {
        ...
        Object.keys(criteriaMap).forEach((criteriaKey) => {
            ...
            if (criteria.type === 'repository') {
                repoColumns.push(criteriaKey);
            }
...
gulp.task('development-column', function (done) {
    const yaml = yaml2json.safeLoad(readFileSync(paths.assets + '/comparison.yml'));
    const development = yaml.criteria.find(c => c.hasOwnProperty('Development')).Development.values;
    const allData = JSON.parse(readFileSync(paths.assets + '/data.json'));
    for (const data of allData) {
        for (const repo of repoColumns) {
            let link = data[repo].plain.trim();
            if (link.startsWith('-')) {
            ...
            if (link.length !== 0) {
                ...
                const labels = getLabels(commits[0], development);
                data.Development = {
    ...
    writeFileSync(paths.assets + '/data.json', JSON.stringify(allData), 'utf8');
    done();
});

gulp.task('build-data', gulp.series('markdown', 'criteria', 'determineColors', 'development-column', 'citation', 'assets'));
// --------------------------------------------------------------->
