...
gulp.task('versionInformation', function () {
    return gulp.src(files.versionInformationExample)
        ...
        .pipe(gulp.dest(paths.assets))
        .pipe(exec('sed -i\'.bak\' "s/§§commitDate§§/$(git log -1 --format=%cd --date=short)/" ' + files.versionInformation))
        .pipe(exec('sed -i\'.bak\' "s/§§commit§§/$(git rev-parse HEAD)/g" ' + files.versionInformation))
        .pipe(exec('sed -i\'.bak\' "s/§§tagDate§§/$(git log --tags -1 --simplify-by-decoration --pretty="format:%cd" --date=short)/g" ' + files.versionInformation))
        .pipe(exec('sed -i\'.bak\' "s/§§tag§§/$(git describe --abbrev=0 --tags)/g" ' + files.versionInformation));
});
