...
gulp.task('update-citation', function () {
    const input = yaml2json.safeLoad(readFileSync(files.config, "utf8"));
    const defaults = yaml2json.safeLoad(readFileSync(files.defaultConfig, "utf8"));
    ...
    gulp.watch([csl, bib], gulp.series('citation', assets));
});
